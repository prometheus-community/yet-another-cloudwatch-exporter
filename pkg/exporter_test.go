...
func TestUpdateMetrics_RDSStorageMetrics(t *testing.T) {
	...
	// Verify RDS storage space metric exists and has correct value (100 GB)
	// The metric aws_rds_storage_space_value is generated by enhanced metrics
	count, err := testutil.GatherAndCount(registry, "aws_rds_storage_space_value")
	require.NoError(t, err)
	require.Equal(t, 1, count, "Should have exactly one aws_rds_storage_space_value metric")

	// Verify the metric value is correct (includes database class and engine name dimensions)
	expectedStorageMetric := `# HELP aws_rds_storage_space_value Help is not implemented yet.
...
`
	err = testutil.GatherAndCompare(registry, strings.NewReader(expectedStorageMetric), "aws_rds_storage_space_value")
	require.NoError(t, err)
	t.Log("Successfully verified RDS storage space metric with value 100 GB")
}
